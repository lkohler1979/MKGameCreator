...
**4.8 Inimigos com comportamento avançado** ✅ — ver `docs/ROADMAP.md` §5 para o detalhamento completo
- [x] `SceneConfig`: campo opcional `enemyBehavior` (`patrulha`/`perseguidor`/`voador`/`saltador`/`atirador`, default `perseguidor`) + `visionRange`/`speed`/`shootIntervalMs`.
- [x] Motores (Plataforma, Coleta de Itens): os 5 comportamentos implementados via `apps/web/src/game/enemy-ai.ts` (funções puras compartilhadas pelos dois motores) — patrulha senoidal, perseguidor/voador só ativam dentro do campo de visão, saltador com arco vertical periódico.
- [x] Motor Atirador: projétil novo (corpo Matter.js sensor, dano ao colidir com o jogador — some no impacto —, expira por tempo ou ao saír da arena).
- [x] Editor (`new-game/edit`): seletor de `enemyBehavior` na Plataforma (visual, toolbar) e na lista do Coleta de Itens (`<select>` adicional) — validado ao vivo no navegador (seleção, destaque visual e persistência no `sceneConfig` salvo).
- [x] Labirinto fica de fora desta fase (pathfinding pelas paredes é escopo maior, já adiado na 4.1) — nenhuma mudança em `maze-game.ts`.
- [x] Testes unitários manuais da lógica pura de `enemy-ai.ts` (patrulha, perseguição com/sem visão, voador 2D, salto, intervalo de disparo, direção do projétil) — todos passaram.
- Decisão confirmada com o usuário: inimigos continuam não-derrotáveis (só se evita, sem golpe/pulo-pra-matar) — "vida" do REQUISITOS.MD não se aplica.
- **Limitação desta rodada**: Docker Desktop não subiu neste ambiente (tentado, processo encerrou sozinho), então não foi possível jogar uma partida real com inimigos em cada comportamento (colisão de projétil, arco de salto, voador perseguindo em 2D) — validado por revisão de código + testes unitários da matemática pura + teste ao vivo do seletor do editor, mas não pelo motor de jogo rodando de ponta a ponta. Recomendado um teste manual rápido em `docker compose up` antes de considerar essa fase "jogada e aprovada".

**4.9 Mais templates de jogo** 🚧 em andamento (Quebra-blocos → Corrida → Air Hockey) — ver `docs/ROADMAP.md` §5 para o detalhamento completo
- [x] Quebra-blocos (`GameTemplate.BRICK_BREAKER`): raquete + bola com física real de quique (deflexão pelo ponto de impacto), tijolos vindos das moedas/obstáculos/powerups marcados no desenho (grade automática, piso mínimo de 12), powerups como tijolos especiais. Migration + `games.ts` + `api.ts` + card no Generate + controles dedicados no Jogar (só esquerda/direita) + `ListEditor` (já genérico, sem mudança extra).
  - Validado: `tsc`/`eslint` limpos; física/colisão/pontuação/powerups/derrota testados rodando o motor isolado (sem backend/Docker) e avançando o `PIXI.Ticker` manualmente — confirmados quebra de tijolo, aplicação de powerup, perda de vida no erro, fim de jogo. Vitória (zerar os tijolos) não repetida em simulação por usar a mesma estrutura de código já validada na derrota.
- [x] Corrida (`GameTemplate.RACE`): corredor infinito estilo "Chrome Dino" — jogador fixo em X, só pula; cenário (obstáculos/moedas/powerups vindos do desenho, posição ignorada) se move da direita pra esquerda com velocidade crescente. Migration + `games.ts` + `api.ts` + card no Generate + botão único de pular no Jogar + timer HUD (reaproveita a condição já usada pelo Coleta de Itens) + `ListEditor` (sem mudança extra).
  - Validado com a mesma técnica da Quebra-blocos: nascimento cíclico dos itens, coleta de moeda, dano de obstáculo, vitória por sobreviver 45s, e **derrota por vidas zeradas confirmada explicitamente** (3 colisões forçadas, vidas 3→2→1→0, `onLose` disparado). Efeitos de powerup testados isoladamente.
- [x] Air Hockey (`GameTemplate.AIR_HOCKEY`): desafio solo numa arena 2D sem gravidade — raquete nas 4 direções rebate um disco livre contra um gol no topo (ponto) e defende o gol de baixo (vida). Obstáculos do desenho viram rebatedores fixos; moedas/powerups coletados pela raquete. Migration + `games.ts` + `api.ts` + card no Generate + controles de 4 direções no Jogar (reaproveita o padrão do Coleta de Itens) + `ListEditor` (sem mudança extra).
  - **Bug real corrigido**: disco perdia velocidade e travava num canto (paredes/rebatedores sem `friction`/`frictionStatic` zerados + perda numérica residual do solver do Matter.js) — corrigido zerando esses valores em todos os corpos sólidos e adicionando um piso mínimo de velocidade (`PUCK_MIN_SPEED`).
  - Validado com a mesma técnica dos templates anteriores: gol marcado, vitória (5 gols), derrota (vidas zeradas), coleta de moeda/powerup pela raquete, movimento nas 4 direções, e velocidade do disco estável após a correção.
